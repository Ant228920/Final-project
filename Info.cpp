...
Info::Info(string newpass, string newname, string newsurname, int newage, string newsex, int newregisnum, string newabout, string newrequirements)
        : pass{newpass},Person(newname, newsurname),age{newage}, sex{newsex}, regisnum{newregisnum}, about{newabout}, requirements{newrequirements}{}
ostream &operator << (ostream &os, const Info &obj){
    os<<obj.pass<<endl<<static_cast<const Person&>(obj)<<obj.age<<endl<<obj.sex<<endl<<obj.regisnum<<endl<<obj.about<<endl<<obj.requirements<<endl;
    return os;
}
